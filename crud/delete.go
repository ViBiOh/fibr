...
func (a *App) Delete(w http.ResponseWriter, r *http.Request, directory string) {
	if r.URL.Path == `/` {
		...
		a.uiApp.Error(w, http.StatusInternalServerError, fmt.Errorf(`Error while deleting: %v`, err))
	} else {
