...
func (a App) generateThumbnail(pathname string) error {
	...
	result, _, _, err := request.Do(ctx, http.MethodPost, fmt.Sprintf(`%s/thumbnail?width=150&height=150&stripmeta=true&noprofile=true&compression=9&type=png`, a.imaginaryURL), file, nil)
	if err != nil {
		return err
	}
