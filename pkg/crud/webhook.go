...
func (a App) createWebhook(w http.ResponseWriter, r *http.Request, request provider.Request) {
	...
	a.rendererApp.Redirect(w, r, fmt.Sprintf("%s/?d=%s#webhook-list", request.SelfURL, request.Display), renderer.NewSuccessMessage("Webhook successfully created with ID: %s", id))
}
...
func (a App) deleteWebhook(w http.ResponseWriter, r *http.Request, request provider.Request) {
	...
	a.rendererApp.Redirect(w, r, fmt.Sprintf("%s/?d=%s#webhook-list", request.SelfURL, request.Display), renderer.NewSuccessMessage("Webhook with id %s successfully deleted", id))
}
